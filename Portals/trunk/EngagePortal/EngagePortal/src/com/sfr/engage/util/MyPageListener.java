        ...
        try {
            ...
            if (phase == 9) {
                ...
                //    if (!AdfFacesContext.getCurrentInstance().isPostback()) {
                    ...
                    List<AccountInfo> accountinfo_list = new ArrayList<AccountInfo>();
                    List<CardGroupInfo> cardgroupinfo_list = new ArrayList<CardGroupInfo>();
                    List<CardInfo> cardinfo_list = new ArrayList<CardInfo>();
                    List<PartnerListInfo> partnerlistinfo_list = new ArrayList<PartnerListInfo>();
                    PartnerListInfo partnerlistobj = new PartnerListInfo();
                    ...
                    for (int i=0; i< user.getRoleList().size();i++)
                    {
                        ...
                        if(user.getRoleList().get(i).getRoleName().equals(Constants.ROLE_WCP_CARD_ADMIN)||(user.getRoleList().get(i).getRoleName().equals(Constants.ROLE_WCP_CARD_B2C_SFR)) || (user.getRoleList().get(i).getRoleName().equals(Constants.ROLE_WCP_CARD_B2B_MGR)))
                        
                        {
                            System.out.println("CARD AMIN as user role name=======>"+user.getRoleList().get(i).getRoleName());
                            ...
                            if(user.getRoleList().get(i).getIdString() != null)
                                                {
                                                    ...
                                                    do
                                                    {
                                                        ...
                                                        System.out.println("Partner id " + idlist + " "+ user.getRoleList().get(i).getIdString().get(idlist));
                                                        int pid_start = user.getRoleList().get(i).getIdString().get(idlist).indexOf("PP");
                                                        System.out.println("pid start index " + pid_start);
                                                        String pid = user.getRoleList().get(i).getIdString().get(idlist).substring(pid_start+2, pid_start+5);
                                                        System.out.println("partner id " + pid);
                                                        partnerobj.setPartnerValue(pid);
                                                        
                                                        if(partnerinfo_list.size() > 0)
                                                        { System.out.println("partner info list size" + partnerinfo_list.size());
                                                          
                                                            for(int k=0; k < partnerinfo_list.size(); k++) {
                                                              System.out.println("temp for loop pid value in list " + partnerinfo_list.get(k).getPartnerValue());}
                                                            boolean addflag = false;
                                                            for(int k=0; k < partnerinfo_list.size(); k++) {
                                                              System.out.println("pid value in list " + partnerinfo_list.get(k).getPartnerValue());
                                                              System.out.println("New pid value to compare" + pid);
                                                        ...
                                                        {
                                                            partnerinfo_list.add(partnerobj);
                                                            System.out.println("value setted in partnere info list" + partnerobj.getPartnerValue());
                    ...
                    System.out.println("Partner id final list size" + partnerinfo_list.size());
                    System.out.println("teetette " + user.getRoleList().get(0).getIdString().get(0));
                    System.out.println("gfgfgf " + partnerinfo_list.get(0).getPartnerValue());
                            ...
                            //                    {
                                                    ...
                                                    {   
                                                      ...
                                                      //      System.out.println("no bindings on template");
                                                        ...
                                                        System.out.println("Partner id in query " + Partnerid);
                                                        ...
                                                        vo1.executeQuery();
                                                           System.out.println("row count from account vo " + vo1.getEstimatedRowCount());


                                                        
                                                        if (vo1.getEstimatedRowCount() != 0) {

                                                            System.out.println("RowCount for account VO  "+vo1.getEstimatedRowCount());
                                                            ...
                                                            while (vo1.hasNext()) {
                                                                ...
                                                                if (currRow != null) {
                                                                    ...
                                                                    if (currRow.getAccountId() != null) {
                                                                          System.out.println("Account id is " + currRow.getAccountId());
                                                                        acc.setAccountNumber(currRow.getAccountId().toString());
                                                                    }
                                                                  
                                                                    addflagaccount = false;
                                                                    for(int k=0; k < accountlist.size(); k++) {
                                                                      System.out.println("account id value in account list " + accountlist.get(k).getAccountNumber());
                                                                      System.out.println("New account id value to compare" + acc.getAccountNumber());
                                                                    ...
                                                                    while (vo2.hasNext()) {
                                                                        ...
                                                                        if (currRowcardgrp != null) {
                                                                            ...
                                                                            if (currRowcardgrp.getCardgroupSeq() != null) {
                                                                                  System.out.println("Cardgroup id is " + currRowcardgrp.getCardgroupSeq());
                                                                                cardgrp.setCardGroupID(currRowcardgrp.getCardgroupSeq().toString());
                                                                            }
                                                                          
                                                                            addflagcardgroup = false;
                                                                            for(int k=0; k < cardgrouplist.size(); k++) {
                                                                              System.out.println("cardgroup id value in cardgroup list " + cardgrouplist.get(k).getCardGroupID());
                                                                              System.out.println("New cardgroup id value to compare" + cardgrp.getCardGroupID());
                                                                        ...
                                                                        String CardgroupID = cardgrp.getCardGroupID();
                                                                        bindings = (DCBindingContainer)BindingContext.getCurrent().getCurrentBindingsEntry();
                                                                        ...
                                                                        vo3.defineNamedWhereClauseParam("cardgroupid",CardgroupID,null);
                                                                            ...
                                                                            while (vo3.hasNext()) {
                                                                                ...
                                                                                if (currRowcard != null) {
                                                                                    ...
                                                                                    for(int k=0; k < cardlist.size(); k++) {
                                                                                      System.out.println("cardgroup id value in cardgroup list " + cardlist.get(k).getCardID());
                                                                                      System.out.println("New cardgroup id value to compare" + card.getCardID());
                            ...
                            if(user.getRoleList().get(i).getIdString() != null) {
                                ...
                                do
                                {
                                         ...
                                         if(user.getRoleList().get(i).getIdString().get(idlist).contains("AC")) {
                                             ...
                                             String accid = user.getRoleList().get(i).getIdString().get(idlist).substring(accid_start+2, accid_start+5);
                    ...
                    //                                             List<AccountInfo> accountlist = new ArrayList<AccountInfo>();
                                           ...
                                           //  part = new PartnerInfo();
                                             ...
                                             String cgid = user.getRoleList().get(i).getIdString().get(idlist).substring(cgid_start+8);
                                             System.out.println("cardgroup id " + cgid);
        ...
        if (userList != null && !userList.isEmpty()) {
            ...
            if (getUser() != null) {
                ...
                if(user.getRolelist() != null && !user.getRolelist().isEmpty())
                {
                    ...
                    System.out.println("RoleList size      -----------" + user.getRoleList().size());
                    System.out.println("RoleList size" + user.getRoleList().get(0).getRoleName());
                    System.out.println("Role of admin" + (Constants.ROLE_WCP_CARD_ADMIN));
...
//            listrole.add(rr);
             ...
             user.setRolelist(Constants.ROLE_WCP_CARD_ADMIN+"|"+Constants.ROLE_WCP_CARD_B2B_MGR);
