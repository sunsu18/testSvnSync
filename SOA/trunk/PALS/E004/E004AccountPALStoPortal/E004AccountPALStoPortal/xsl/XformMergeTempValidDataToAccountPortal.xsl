...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 11.1.1.6.0(build 111214.0600.1553) AT [THU OCT 30 17:30:46 IST 2014]. -->
?>
<xsl:stylesheet version="1.0"
                ...
                exclude-result-prefixes="xsi xsl db plt wsdl tns xsd ns0 top xp20 bpws bpel bpm ora socket mhdr oraext dvm hwf med ids xdk xref bpmn ldap">
  <xsl:template match="/">
    <top:PrtAccountCollection>
      <xsl:for-each select="/db:SelectValidDataTempAccountPortalDBAdapterOutputCollection/db:SelectValidDataTempAccountPortalDBAdapterOutput">
        <top:PrtAccount>
          ...
          <top:accountName>
            <xsl:if test="db:ACCOUNT_NAME/@xsi:nil">
              <xsl:attribute name="xsi:nil">
                <xsl:value-of select="db:ACCOUNT_NAME/@xsi:nil"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:value-of select="db:ACCOUNT_NAME"/>
          </top:accountName>
          <top:customerSegment>
            <xsl:if test="db:CUSTOMER_SEGMENT/@xsi:nil">
              <xsl:attribute name="xsi:nil">
                <xsl:value-of select="db:CUSTOMER_SEGMENT/@xsi:nil"/>
              </xsl:attribute>
            </xsl:if>
            <xsl:value-of select="db:CUSTOMER_SEGMENT"/>
          </top:customerSegment>
        </top:PrtAccount>
      </xsl:for-each>
    </top:PrtAccountCollection>
