...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 11.1.1.6.0(build 111214.0600.1553) AT [WED MAY 07 19:12:58 IST 2014]. -->
?>
  ...
  <xsl:template match="/">
    <ns2:EngageSoaCardgroupErrorlogCollection>
      <xsl:for-each select="/ns1:CardGroup/ns1:CardGroupData">
        <xsl:if test='((((((normalize-space(ns1:ACCOUNT_ID) = "") or (string-length(normalize-space(ns1:ACCOUNT_ID)) != 10)) or (string-length(normalize-space(ns1:PARTNER_ID)) != 8)) or (string-length(normalize-space(ns1:CARDGROUP_MAIN_TYPE)) != 3)) or (string-length(normalize-space(ns1:CARDGROUP_SUB_TYPE)) != 3)) or (string-length(normalize-space(ns1:CARDGROUP_SEQ)) != 5)) or (string-length(normalize-space(ns1:CARDGROUP_DESCRIPTION)) > 45)'>
