...
BEGIN   
 ...
 BLOCK_DATE=d.blkdt, BLOCK_TIME=d.blkt, BLOCK_LEVEL=d.blkl, MANUFACTURED_DATE=d.mfd, ROUTEX_CUSTOMER_NUMBER=d.rtctn, INVOICE_ADDR1=d.invad1, INVOICE_ADDR2=d.invad2, INVOICE_POSTAL_CODE=d.invpc, INVOICE_CITY=d.invct, ADDR_SEQ=d.adsq, INVOICE_COUNTRY=d.invcnt, INVOICE_ADDR_LEVEL=d.invadl, INVOICE_ADDR_TYPE=d.invadt, MODIFIED_BY=d.mdb, MODIFIED_DATE=d.mdt
  ...
  ---Added Condition for Driver
   IF (SUBSTR(CARDTYPE,0,1) = '2' OR CARDTYPE = 'CHA' OR CARDTYPE = 'SJN' OR CARDTYPE like 'S_K') 
  THEN
        MERGE into PRT_DRIVER_INFORMATION
        USING (select COUNTRYCODE cc,PRTCARDPK ppk from DUAL) PDI on (PRT_DRIVER_INFORMATION.COUNTRY_CODE = PDI.cc and PRT_DRIVER_INFORMATION.CARD_NUMBER = PDI.ppk)
        when not matched then insert(COUNTRY_CODE,PRT_DRIVER_INFORMATION_PK,ACCOUNT_NUMBER,CARD_NUMBER,DRIVER_NUMBER,DRIVER_NAME,MODIFIED_BY,MODIFIED_DATE) VALUES(COUNTRYCODE,PRT_DRIVER_INFORMATION_SEQ.NEXTVAL,(select ACCOUNT_ID from PRT_CARDGROUP prtcg where (prtcg.partner_id = PARTNERID AND prtcg.cardgroup_main_type=CARDGROUPMAINTYPE AND prtcg.CARDGROUP_SUB_TYPE=CARDGROUPSUBTYPE AND prtcg.CARDGROUP_SEQ=CARDGROUPSEQ AND prtcg.COUNTRY_CODE = COUNTRYCODE)),PRTCARDPK,SUBSTR (CARDEMBOSSNO,-10,30),PRT_DRIVER_SEQ_DRIVER_NAME.NEXTVAL,MODIFIEDBY,MODIFIEDDATE);
end if;

  ---
     ---Added Condition for Vehicle
   IF (CARDTYPE = 'VEH' OR CARDTYPE LIKE 'BI_' OR CARDTYPE LIKE 'BK_' OR CARDTYPE LIKE 'B_K') 
  THEN
		MERGE INTO PRT_TRUCK_INFORMATION
		USING (select COUNTRYCODE cc,PRTCARDPK ppk from DUAL) PTI on (PRT_TRUCK_INFORMATION.COUNTRY_CODE = PTI.cc and PRT_TRUCK_INFORMATION.CARD_NUMBER = PTI.ppk)
		when not matched then insert(COUNTRY_CODE,PRT_TRUCK_INFORMATION_PK,ACCOUNT_NUMBER,CARD_NUMBER,VEHICLE_NUMBER,INTERNAL_NAME,MODIFIED_BY,MODIFIED_DATE) VALUES(COUNTRYCODE,PRT_TRUCK_INFORMATION_SEQ.NEXTVAL,(select ACCOUNT_ID from PRT_CARDGROUP prtcg where (prtcg.partner_id = PARTNERID AND prtcg.cardgroup_main_type=CARDGROUPMAINTYPE AND prtcg.CARDGROUP_SUB_TYPE=CARDGROUPSUBTYPE AND prtcg.CARDGROUP_SEQ=CARDGROUPSEQ AND prtcg.COUNTRY_CODE = COUNTRYCODE)),PRTCARDPK,CARDEMBOSSNO,PRT_VEHICLE_SEQ_VEHICLE_NAME.NEXTVAL,MODIFIEDBY,MODIFIEDDATE);
end if;
  ---
